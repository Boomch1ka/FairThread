...
fun CheckoutScreen(uid: String, navController: NavController, viewModel: CheckoutViewModel = viewModel()) {
    ...
    FairThreadScaffold(navController, title = stringResource(id = R.string.checkout)) { paddingValues ->
        Column(
            modifier = Modifier
                .fillMaxSize()
            ...
            Text(stringResource(id = R.string.checkout), style = MaterialTheme.typography.h4)
            ...
            Text("${stringResource(id = R.string.total)}: R${String.format(Locale.getDefault(), "%.2f", total)}", style = MaterialTheme.typography.h5)
