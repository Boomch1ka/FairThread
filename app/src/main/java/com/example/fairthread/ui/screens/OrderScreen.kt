...
fun OrderScreen(uid: String, navController: NavController, viewModel: OrderViewModel = viewModel()) {
    ...
    FairThreadScaffold(navController, title = stringResource(id = R.string.my_orders)) { paddingValues ->
        Column(
            modifier = Modifier
                .fillMaxSize()
                    ...
                    items(orders) { order ->
                        Card(modifier = Modifier.fillMaxWidth(), elevation = 4.dp) {
                            Column(modifier = Modifier.padding(16.dp)) {
                                Text("${stringResource(id = R.string.order_id)}: ${order.id}", style = MaterialTheme.typography.h6)
                                ...
                                Text("${stringResource(id = R.string.total)}: R${String.format(Locale.getDefault(), "%.2f", total)}", style = MaterialTheme.typography.h6)
                            }
                        }
                    }
                }
